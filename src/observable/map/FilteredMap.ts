...
type Filter<K,V> = (value: V, key: K) => boolean

export class FilteredMap<K,V> extends BaseObservableMap<K,V> {
    private _source: BaseObservableMap<K,V>
    private _filter: Filter<K,V> | null
    private _included: Map<K,boolean> | null = null
    private _subscription: (() => void) | null = null;

    constructor(source: BaseObservableMap<K,V>, filter: Filter<K,V> | null = null) {
        super();
        this._source = source;
        this._filter = filter;
    }
     ...
     * reapply the filter
     */
    _reapplyFilter(silent: boolean = false): void {
        if (this._filter) {
    ...
    _emitForUpdate(wasIncluded: boolean, isIncluded: boolean, key: K, value: V, params: any = null): void {
...
class FilterIterator<K,V> {
    ...
    private _sourceIterator: IterableIterator<[K,V]>

    constructor(map: BaseObservableMap<K,V>, _included: Map<K,boolean>) {
...
export function tests() {
    return {
        ...
        "emits must trigger once": assert => {
            ...
            oddMap.subscribe(defaultObserverWith({
                onAdd() {
                    count_add += 1;
                },
                onRemove() {
